...
# and returns a tuple with 3 elements - the first, third
# and second element from the last for the given array.
# One important thing worth pointing out is that
# you need to use index in order to extract elements from the array.
# Pay attention, index counting starts from 0, not from 1.
# Which means that if you need to get the first element from the array elements,
...
def easy_unpack(elements: tuple) -> tuple:
    first = elements[0]
    third = elements[2]
    minus_second = elements[-2]
    return first, third, minus_second
